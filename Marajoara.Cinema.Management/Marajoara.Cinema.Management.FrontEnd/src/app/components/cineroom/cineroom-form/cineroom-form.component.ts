...
export class CineroomFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.cineRoomForm = new FormGroup({
      cineRoomID: new FormControl( this.cineRoomData? this.cineRoomData.cineRoomID: ''),
      name: new FormControl( this.cineRoomData? this.cineRoomData.name: '',[Validators.required]),      
      seatsColumn: new FormControl(this.cineRoomData? this.cineRoomData.seatsColumn:5, [Validators.required]),
      seatsRow: new FormControl(this.cineRoomData? this.cineRoomData.seatsRow:4, [Validators.required,  Validators.min(1)]),
      totalSeats: new FormControl(this.cineRoomData? this.cineRoomData.seatsRow! *this.cineRoomData.seatsColumn!  :20, [Validators.required,  Validators.min(20), Validators.max(100)]),
    });    
  }
...
export const seatCountValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
